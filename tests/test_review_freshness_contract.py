...
+            sources=[source("axiom-proof", run_id=101), source("deterministic-review", run_id=102)],
+        )
+
...
+        self.assertIn("AX-REV-FRESH-CONTRACT-2001", {item.code for item in validate_freshness_envelope(envelope)})
+
...
+        )
+        self.assertIn("AX-REV-FRESH-0102", {item.code for item in validate_freshness_envelope(envelope)})
+
